									   ...
									   unsigned char *pData)
{
	...
	pFirmware->mpConfigurations = kmalloc(sizeof(struct TConfiguration) * pFirmware->mnConfigurations, GFP_KERNEL);
							  ...
							  unsigned char *pData)
{
	...
	pFirmware->mpCalibrations = kmalloc(sizeof(struct TCalibration) * pFirmware->mnCalibrations, GFP_KERNEL);
											 ...
											 int nReg, int *pnValue)
{
	...
	for (i = 0; i < pBlock->mnCommands;) {
		...
		if ((nOffset < 0x7f) || (nOffset == 0x81))
			...
			if ((len - 1) % 4)
				i++;
			if ((nBook != TAS2557_BOOK_ID(nReg)) || (nPage != TAS2557_PAGE_ID(nReg)))
				continue;
