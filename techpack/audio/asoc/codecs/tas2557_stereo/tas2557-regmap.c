									...
									unsigned char nBook, unsigned char nPage)
{
	...
	if (chn == channel_broadcast) {
		...
		pTAS2557->mnLCurrentPage = 0;
		pTAS2557->mnRCurrentPage = 0;
		nResult = tas2557_i2c_write_device(pTAS2557, TAS2557_BROADCAST_ADDR, TAS2557_BOOKCTL_REG, nBook);
		if (nResult < 0)
			goto end;
		pTAS2557->mnLCurrentBook = nBook;
		pTAS2557->mnRCurrentBook = nBook;
		nResult = tas2557_i2c_write_device(pTAS2557, TAS2557_BROADCAST_ADDR, TAS2557_BOOKCTL_PAGE, nPage);
		if (nResult >= 0) {
			pTAS2557->mnLCurrentPage = nPage;
			pTAS2557->mnRCurrentPage = nPage;
		}
								  ...
								  unsigned int nRegister, u8 *pData, unsigned int nLength)
{
	...
	if (nResult >= 0) {
		reg = TAS2557_PAGE_REG(nRegister);
		if (chn & channel_left) {
			nResult = tas2557_i2c_bulkwrite_device(pTAS2557, pTAS2557->mnLAddr, reg, pData, nLength);
			if (nResult < 0)
				goto end;
		}

		if (chn & channel_right) {
			nResult = tas2557_i2c_bulkwrite_device(pTAS2557, pTAS2557->mnRAddr, reg, pData, nLength);
			if (nResult < 0)
				goto end;
		}
								   ...
								   unsigned int nValue)
{
	...
	if (chn & channel_left) {
		nResult = tas2557_i2c_update_bits(pTAS2557, pTAS2557->mnLAddr, TAS2557_PAGE_REG(nRegister), nMask, nValue);
		if (nResult < 0)
			goto end;
	}

	if (chn & channel_right)
		nResult = tas2557_i2c_update_bits(pTAS2557, pTAS2557->mnRAddr, TAS2557_PAGE_REG(nRegister), nMask, nValue);
...
static int tas2557_i2c_probe(struct i2c_client *pClient, const struct i2c_device_id *pID)
{
	...
	if (gpio_is_valid(pTAS2557->mnRightChlGpioINT)) {
		if (pTAS2557->mnLeftChlGpioINT != pTAS2557->mnRightChlGpioINT) {
			...
			nResult = request_threaded_irq(pTAS2557->mnRightChlIRQ, tas2557_irq_handler, NULL,
										 IRQF_TRIGGER_HIGH | IRQF_ONESHOT, pClient->name, pTAS2557);
			if (nResult < 0) {
				dev_err(pTAS2557->dev, "request_irq failed, %d\n", nResult);
				goto err;
